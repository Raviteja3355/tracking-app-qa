...
const CONTACT_REASONS_DISPLAY: Array<{ value: ContactReason; labelKey: string }> = [
...
function Modal({ message, onClose, btnLabel }: { message: string; onClose: () => void; btnLabel: string }) {
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60">
      <div className="mx-4 max-w-sm rounded-[20px] bg-white p-8 text-center" style={{ boxShadow: '0 0 40px 0 #D4EFF7' }}>
        <p className="text-[15px] text-uni-black">{message}</p>
        <button
          onClick={onClose}
          className="mt-5 rounded-[10px] px-10 py-3.5 text-[16px] font-medium text-white transition-opacity hover:opacity-90"
          style={{ background: 'linear-gradient(to top, #FF6A13 0%, #FF8F1C 100%)' }}
        >
          {btnLabel}
        </button>
      </div>
...
export default function CustomerSupport() {
  ...
  useEffect(() => {
    axios
      ...
      .then(({ data }) => {
        ...
        data.data.forEach((item) => { rel[item.state] = item.cities })
        setRelationship(rel)
  ...
  async function handleSend() {
    if (!reason || !incident || !firstName || !lastName || !email || !content || !trackingNumber) {
    ...
    try {
      ...
      if (attachment) {
        ...
        form.append(
          country === 'Canada' ? 'custom_fields[cf_canada]' : 'custom_fields[cf_usa]',
          warehouse,
        )
        form.append('custom_fields[cf_tracking_number]', trackingNumber || '000')
      ...
      const msg =
        err instanceof Error
          ? err.message
          : (err as { response?: { data?: { status?: string } } })?.response?.data?.status === 'SUCCESS'
          ? 'SUCCESS'
          : null
      if (msg === 'SUCCESS') {
        setModal(t('successTicketCreated'))
  ...
  return (
    ...
    >
        ...
        <div className="mx-auto max-w-150">
          ...
          >
            ...
            <div className="mb-5">
              ...
              <select
                value={reason}
                onChange={(e) => onReasonChange(e.target.value as ContactReason)}
            ...
            <div className="mb-7">
              <div className="mb-2.5 flex items-center justify-between gap-2">
                <div className="flex items-center gap-1">
                  <span className="text-[16px] font-semibold text-uni-black">{t('labelHowCanWeHelp')}</span>
      ...
      {modal && <Modal message={modal} onClose={() => setModal(null)} btnLabel={t('btnOkay')} />}
    </section>
  )
}
